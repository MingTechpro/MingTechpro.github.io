document.addEventListener("DOMContentLoaded", () => {
    ...
    const alertInfo = (ele, text) => {
      if (GLOBAL_CONFIG.Snackbar !== undefined) {
        ...
        const finalLeft = Math.max(
          halfWidth + 10,
          Math.min(window.innerWidth - halfWidth - 10, centerLeft),
        );
        ...
        const topValue = shouldShowBelow
          ? buttonRect.top + scrollTop + buttonRect.height + 10
          : normalTop;
  ...
  const scrollFnToDo = () => {
    ...
    btf.addEventListenerPjax(
      window,
      "resize",
      btf.throttle(updateHeaderPositions, 200),
    );
